...
////////////////////////////////////////////////////////////////////////////////////
class uart_config extends uvm_object; /////configuration of env as acrive or passive
...
{rand_baud_1_stop = 0, rand_length_1_stop = 1, length5wp = 2, length6wp = 3, length7wp = 4, length8wp = 5,
length5wop = 6, length6wop = 7, length7wop = 8, length8wop = 9,
rand_baud_2_stop = 11, rand_length_2_stop = 12} oper_mode;
...
class rand_baud_with_stop extends uvm_sequence#(transaction);
  ...
  transaction tr; //instance to store this tx, each sequence will have its local instance
  ...
  virtual task body();
    repeat(5)
      begin
        ...
        tr.op     = length5wp;
        tr.rst       = 1'b0;
        tr.tx_data   = {3'b000, tr.tx_data[7:3]}; //reverse LSB and add shift right the MSB 
        tr.length = 5;
        tr.tx_start  = 1'b1;
        tr.rx_start  = 1'b1;
        tr.parity_en = 1'b1;
  ...
  virtual task body();
    repeat(5)
      begin
        ...
        tr.tx_start  = 1'b1;
        tr.rx_start  = 1'b1;
        tr.parity_en = 1'b0;  //Parity is different
        tr.stop2     = 1'b0;
        finish_item(tr);
      end
  endtask
...
class driver extends uvm_driver #(transaction);
  ...
  virtual uart_if vif; //key to access virtual interface
  transaction tr;      //instance of an object, IS THIS INSTANCE SAME AS INSTANCE DEFINED FOR STIMULUS, ELSE HOW CAN YO REPLICATE THIS 
 ...
 virtual function void build_phase(uvm_phase phase);
  ...
  task drive();
   ...
   forever begin
       ...
       seq_item_port.get_next_item(tr);  //access grant from sequencer and take values stored in "tr" instance 
                                   
                            vif.rst         <= 1'b0;          //apply data form sequencer to DUT
    ...
    forever begin
      ...
      if(vif.rst)
        ...
        send.write(tr);
         ...
         begin
         @(posedge vif.tx_done);  //wait and sample all of the signals at data bus output of the reciever
...
`uvm_component_utils(sco)
  ...
  //pronting data recieved from the monitor
...
virtual function void connect_phase(uvm_phase phase);
super.connect_phase(phase); //connet aport of mpnitor to the a export of scoreboard
